...
expectError(
    new OpenAiChatClient({ deploymentId: 'test', modelName: 'test' }).invoke('Test')
);

expectError(
    new OpenAiChatClient({ modelName: 'my-cool-chat-model' })
);

expectError(
    new OpenAiChatClient({ deploymentId: 'test', apiKey: 'test' })
);
...
expectType<Promise<number[]>>(
    new OpenAiEmbeddingClient({ modelName: 'text-embedding-3-large' }).embedQuery('test')
);
