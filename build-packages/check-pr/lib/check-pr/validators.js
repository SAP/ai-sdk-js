...
async function validatePreamble(preamble) {
    ...
    if (!groups) {
        return setFailed('Could not parse preamble. Ensure it follows the conventional commit guidelines.');
    }
...
function validatePostamble(title) {
    if (!title || !title.trim().length) {
        return setFailed('PR title does not have a title after conventional commit preamble.');
    }
    if (title[0] !== ' ') {
...
async function extractChangesetFileContents() {
    ...
    const changesetMarkdownFiles = changeSetFiles.filter(file => /^\.changeset\/(pre\/)?[^/]+\.md$/.test(file));
    const fileContents = await Promise.all(changesetMarkdownFiles.map(file => readFile(file, 'utf-8')));
    return fileContents;
}
export function validateChangesets(preamble, commitType, isBreaking, fileContents) {
