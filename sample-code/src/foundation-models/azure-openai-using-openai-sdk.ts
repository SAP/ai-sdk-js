...
/**
 * Pass a PDF file inline as base64 and use it as context in a Responses API call.
 * @returns The answer grounded in the file content.
 */
export async function responsesApiWithFile(): Promise<string | undefined> {
  const client = await createAzureOpenAIClient({
  ...
  const response = await client.responses.create({
    instructions: 'Transcribe the text content of the PDF document.',
    input: [
      {
        type: 'message',
        role: 'user',
        content: [
          ...
          { type: 'input_text', text: 'Summarize the content of the file.' }
        ]
      }
    ]
  });
