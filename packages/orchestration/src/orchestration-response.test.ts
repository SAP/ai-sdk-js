...
describe('OrchestrationResponse', () => {
  ...
  it('should convert file content items in getAllMessages from orchestration to SDK format', () => {
    const localMockResponse = {
      ...mockResponse,
      intermediate_results: {
        ...mockResponse.intermediate_results,
        templating: [
          {
            role: 'user',
            content: [
      ...
      final_result: {
        ...
        choices: [
          {
            index: 0,
            message: { role: 'assistant', content: 'Summary of the document.' },
            finish_reason: 'stop'
          }
        ]
      }
    ...
    const localResponse = new OrchestrationResponse({
      ...rawResponse,
      data: localMockResponse
    });

    const messages = localResponse.getAllMessages();
