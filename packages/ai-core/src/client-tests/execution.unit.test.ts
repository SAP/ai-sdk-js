...
describe('execution unit tests', () => {
  ...
  it('get execution parses a successful response', async () => {
    ...
    expect(execution.configurationId).toEqual(
      'e0a9eb2e-9ea1-43bf-aff5-7660db166676'
    );
    expect(execution.configurationName).toEqual(
      'spam-detection-execution-config-0'
    );
    ...
    expect(outputArtifacts[0].id).toEqual(
      'be0d728f-1cb2-4ff4-97ad-45c54ac592f6'
    );
    expect(outputArtifacts[0].kind).toEqual('model');
    expect(outputArtifacts[0].url).toEqual(
      'ai://default/eab289226fe981da/classifier-model-output'
    );
  });

  it('post execution parses a successful response', async () => {
    nock(destination.url).post('/lm/executions').reply(
      200,
      {
        id: 'eab289226fe981da',
