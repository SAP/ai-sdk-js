...
export class OpenAiClient implements BaseClient<BaseLlmParameters> {
  ...
  async chatCompletion(
    ...
    const response = await executeRequest(
      { url: `/inference/deployments/${data.deploymentConfiguration.deploymentId}/chat/completions`, apiVersion },
      data,
      requestConfig
    );
  ...
  async embeddings(
    ...
    const response = await executeRequest(
      { url: `/inference/deployments/${data.deploymentConfiguration.deploymentId}/embeddings`, apiVersion },
      data,
      requestConfig
    );
    return response.data;
