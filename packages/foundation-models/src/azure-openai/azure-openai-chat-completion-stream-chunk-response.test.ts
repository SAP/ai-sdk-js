...
describe('OpenAI chat completion stream chunk response', () => {
  ...
  beforeAll(async () => {
    ...
    azureOpenAiChatCompletionStreamChunkResponses = {
      tokenUsage: new AzureOpenAiChatCompletionStreamChunkResponse(mockResponses.tokenUsage),
      finishReason: new AzureOpenAiChatCompletionStreamChunkResponse(mockResponses.finishReason),
      deltaContent: new AzureOpenAiChatCompletionStreamChunkResponse(mockResponses.deltaContent)
    };
  });

  it('should return the chat completion stream chunk response', () => {
    expect(azureOpenAiChatCompletionStreamChunkResponses.tokenUsage.chunk).toStrictEqual(mockResponses.tokenUsage);
    expect(azureOpenAiChatCompletionStreamChunkResponses.finishReason.chunk).toStrictEqual(mockResponses.finishReason);
    expect(azureOpenAiChatCompletionStreamChunkResponses.deltaContent.chunk).toStrictEqual(mockResponses.deltaContent);
  });

  it('should get token usage', () => {
    expect(azureOpenAiChatCompletionStreamChunkResponses.tokenUsage.getTokenUsage()).toMatchObject({
  ...
  it('should return finish reason', () => {
    expect(azureOpenAiChatCompletionStreamChunkResponses.finishReason.getFinishReason()).toBe('stop');
  });

  it('should return delta content with default index 0', () => {
    expect(azureOpenAiChatCompletionStreamChunkResponses.deltaContent.getDeltaContent()).toBe(' is');
  });
});